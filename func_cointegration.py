...
def calculate_half_life(spread):
    ...
    spread_lag = df_spread.spread.shift(1) # the above row
...
# Updated version in Lesson 34 will casue error. Do not use it!
...
def store_cointegration_results(df_market_prcies):
    
    # Initialize 
    markets = df_market_prcies.columns.to_list() # Get a list of markets (cryptos)
    criteria_met_pairs = []
    
    # Find coitegrated pairs
    # Start with our base pair
    for index, base_market in enumerate(markets[:-1]):
        series_1 = df_market_prcies[base_market].values.astype(float).tolist()
        
        # Get quote pair
        for quote_market in markets[index +1:]:
            series_2 = df_market_prcies[quote_market].values.astype(float).tolist()
    ...
    df_criteria_met.to_csv(f"cointegrated_pairs.csv")
